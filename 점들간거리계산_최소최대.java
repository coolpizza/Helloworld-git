public class Main {
    ...
    static void findMinAndMaxDistance(Point[] points, Point[] selectedPoints, int index, int totalDistance, int selectedCount, Point origin) {
		...
		findMinAndMaxDistance(points, selectedPoints, index + 1, 
		totalDistance + Math.abs(points[index].getRow() - origin.getRow()) + Math.abs(points[index].getColumn() - origin.getColumn()), selectedCount + 1, origin);
        
		selectedPoints[selectedCount] = null;
        
		findMinAndMaxDistance(points, selectedPoints, index + 1, 
		totalDistance, selectedCount, origin);
    }
